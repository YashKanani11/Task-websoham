...
const AuthorCard = ({ img, name, bg, width = 109, fontSize = 12 }) => {
  const bgColor = bg ? bg : 'rgba(109, 86, 101, 0.5)';

  return (
    <div className={`w-[100%] px-6 m-auto box-border mt-32 rounded-full flex relative group cursor-pointer text-black`}
    style={{backgroundColor:bgColor}}>
      <div className='w-[20%] py-1 box-border rounded-full'>
        <img className='h-14 object-fill rounded-full' src={img} />
      </div>
      <div className='w-[50%] mt-2'>
        <p className={`text-[${fontSize}px]`}>AUTHOR</p>
        <p className={`font-bold text-[${fontSize + 5}px]`}>{name}</p>
      </div>
      <div className='w-[40%] absolute -right-2 group-hover:-right-5 transition-all'>
        <svg className='mt-6' width={width} height="15" viewBox="0 0 119 15" fill="none" xmlns="http://www.w3.org/2000/svg">
